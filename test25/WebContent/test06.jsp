...
<%
	...
	//Servlet application = pageContext.getServletConfig();
	
	//Servlet에서 exception 객체 생성하기
	//Exception exception = pageContext.getException();
	
	//서블릿에서 jsp 객체 생성하기
	//JspWriter jsOut = pageContext.getOut();
...
<%
	...
	out.println("<h3>Session(세션)은 로그아웃 전 또느 브라우저를 종료하기 전까지 그 정보다 유지된다.</h3>");
	...
	out.println("<h3> 전송된 값은 그 정보다 목적지까지만 유지되며, 다른 페이지에서는 해당 정보를 확인할 수 없음</h3>");
	out.println("<p>웹 환경에서의 Scope : 그 정보가 어디 또는 언제까지 유지</p>");
	out.println("Page < trans(request & response) < session < application");

	//page.setAttribute("name", "김응원"); //모든 8대 객체에서 값을 저장하는 경우는 setAttribute("필드명","값")페이지가 달라지면 인식 안되는 명령 scope, 객체에 값을 저장
	//response.getAttribute("name", "김응원"); //모든 8대 객체에서 값을 가져올 경우는 getAttribute("필드명")단, request 객체의 값을 가져올 경우는 getParameter("파라미터명") 객체에 값을 가져오는 것
